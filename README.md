...
### ufBGCtoolbox: bgc_dom_div sample
...
```
Run bgc_dom_div in sample mode
```
for i in $( seq -s" " 1  3); do

  sudo ./run_bgc_dom_div.bash sample \
  example/pe_bgc_dom"${i}".gz \
  example/sim_meta_oms-"${i}"_redu_r1.fasta.gz \
  example/sim_meta_oms-"${i}"_redu_r2.fasta.gz \
  example/out_dom_div "${i}".test \
  --blast t \
  --identity 0.5 \
  --plot_tree t \
  --only_rep t \
...
### ufBGCtoolbox: bgc_dom_div merge
...
Run bgc_dom_div in merge mode for Condensation
```
sudo ./run_bgc_dom_div.bash merge \
example/out_dom_div1,example/out_dom_div2,example/out_dom_div3 \
example/out_dom_merged_div_Condensation \
--domain Condensation \
--num_iter 50 \
--sample_increment 20 \
--plot_rare_curve t \
...
Run bgc_dom_div in merge mode for PKS_KS
```
sudo ./run_bgc_dom_div.bash merge \
example/out_dom_div1,example/out_dom_div2,example/out_dom_div3 \
example/out_dom_merged_div_PKS_KS \
--domain PKS_KS \
--num_iter 50 \
--sample_increment 20 \
--plot_rare_curve t \
