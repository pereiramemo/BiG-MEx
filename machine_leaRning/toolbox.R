...
regression_model_fun <- function(bgc, data, regression_method, class2dom, seed = 111) {
  ...
  for (b in bgc) {
     ...
     ######################################
     predictors_train <- data.frame(data_train[,X_DOMS])
     colnames(predictors_train) <- X_DOMS
     
     response_train <- data.frame(data_train[,b])
     colnames(response_train) <- b
     
     predictors_test <- data.frame(data_test[,X_DOMS] )
     colnames(predictors_test) <- X_DOMS
     
     response_test <- data.frame(data_test[,b])
     colnames(response_test) <- b
...
double_models_fun <- function(bgc, data, binary_method, regression_method, class2dom, seed = 111) {
  ...
  for (b in bgc) {
    ...
    if ( sum(!subset) == 0 ) {
      ...
      ##############################################
      predictors_present_train  <- data.frame(data_present_train[,X_DOMS])
      colnames(predictors_present_train) <- X_DOMS
    
      response_present_train <- data.frame(data_present_train[,b])
      colnames(response_present_train) <- b
    
      predictors_present_test  <- data.frame(data_present_test[, X_DOMS])
      colnames(predictors_present_test) <- X_DOMS
    
      response_present_test  <- data.frame(data_present_test[,b])
      colnames(response_present_test) <- b
      ...
      if (binary_method == "rf") {
        model_c <- randomForest( factor(response_binary_train) ~ ., 
                                 data = predictors_train,  ntree = 1000 )
      } 
