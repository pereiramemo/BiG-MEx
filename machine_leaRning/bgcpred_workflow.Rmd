...
#data_train <- data_train %>% select(-strK_like2)

for (b in bgc_classes_all) {

  y <- data_train[,b, drop = T ]
  domains_x <- get_domains(b)
  domains_x <- domains_x[ domains_x %in% colnames(data_train) ]
  x <- data_train[,domains_x, drop = F ]
...
for (b in bgc_classes_all) {
  ...
  x <- data_test[,domains_x, drop = F ]
  
  domains_x_model <- names(bgc_model[[b]]$regression_model$coefficients)[-1]
  zeros_ncol <- domains_x_model[! domains_x_model %in% domains_x ] 
  if (length(zeros_ncol) > 0 ) {
    zeros2add <- matrix(0, nrow = dim(x)[1], ncol = length(zeros_ncol)) %>% as.data.frame()
    colnames(zeros2add) <- zeros_ncol
    x <- cbind(x,zeros2add)
  }
