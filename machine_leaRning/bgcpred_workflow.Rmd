...
URL_train_data_domains <- paste(URL,"OMs_metagenomes_dom_annot_wide.tsv", sep = "")
train_data_domains <- read_tsv(file = URL_train_data_domains, col_names = TRUE)

URL_test_data_domains <- paste(URL,"TGs_metagenomes_dom_annot_wide.tsv", sep = "")
test_data_domains <- read_tsv(file = URL_test_data_domains, col_names = TRUE)

URL_train_data_classes <- paste(URL,"OMs_ref_genomes_class_cov_wide.tsv", sep = "")
train_data_classes <- read_tsv(file = URL_train_data_classes, col_names = TRUE) 

URL_test_data_classes <- paste(URL,"TGs_ref_genomes_class_cov_wide.tsv", sep = "")
...
Next, we will extract the residuals and fitted values from the MLR models in the bgc_model list, to check MLR assumptions through the visualization of different plots.
...
ggplot(res_df, aes(res)) +
  geom_histogram(aes(fill = bgc), alpha = 0.8) +  
...
2. Homoscedasticity
Here we plot the residuales vs. the fitted values. The idea is to check if the variability of the residuals stays constant as the fitted values increase.

```{r, warning=FALSE}
pred_df2 <- lapply(bgc_model,"[[", "regression_model") %>%
            lapply(., fitted) %>%
            plyr::ldply(., cbind)

colnames(pred_df2) <- c("bgc","pred")
pred2res_df <- data.frame(bgc = pred_df2$bgc, pred = pred_df2$pred, res = res_df$res)

# plot
ggplot(pred2res_df, aes(x = pred, y = res)) +
...
3. Linearity
Lastly, we check if there is a linar relationship between predictor and response variables. To do this, we will plot the residuales vs. the predictor variables. Given that we will have to to a plot for each domain, we only show the plots for the nrps.  

```{r, warning=FALSE}
res_redu_df <- res_df %>% filter(bgc == "nrps")
domains_x <- get_domains(target_class)
domains_x <- domains_x[ domains_x %in% colnames(train_data_domains) ]
