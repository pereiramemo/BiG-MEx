...
if [[ "$?" -ne "0" ]]; then
  echo "Generating ${OUT_DIR}/query_clean.fasta failed"
  exit 1
fi 
...
if [[ "$?" -ne "0" ]]; then
  echo "Generating ${OUT_DIR}/${DOMAIN}_tmp.csv failed"
  exit 1
fi  
...
if [[ "$?" -ne "0" ]]; then
  echo "Naming ${OUT_DIR}/${DOMAIN}_query_info.csv failed"
  exit 1
fi  
...
if [[ "$?" -ne "0" ]]; then
  echo "Removing ${OUT_DIR}/${DOMAIN}_query_edpl.csv failed"
  exit 1
fi  
